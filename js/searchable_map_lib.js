...
var SearchableMapLib = {
  ...
  initialize: function(options){
    ...
    SearchableMapLib.map_centroid = options.map_centroid || [40.7128, -74.0060],
    SearchableMapLib.defaultZoom = options.defaultZoom || 11,
    SearchableMapLib.filePath = options.filePath || "data/Affordable_Housing_Production_by_Building.csv",
    ...
    if (!SearchableMapLib.map) {
      ...
      SearchableMapLib.info.update = function (props) {
        if (props) {
          ...
          this._div.innerHTML = 'Hover over an ' + SearchableMapLib.recordName;
        }
      };
  ...
  createSQL: function() {
    ...
    if (name_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
          return r.properties["Project Name"].toLowerCase().indexOf(name_search.toLowerCase()) > -1;
        });
    }
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["Reporting Construction Type"])} );
        },
  ...
  getIcon: function(type){
    if (type == "New Construction") return redIcon;
    if (type == "Preservation") return blueIcon;
    return greenIcon;
  },
